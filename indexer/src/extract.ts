...
export function canonicalPath(value: string): string {
  ...
  // .NET expands 8.3 aliases even for missing files. Resolve an existing
  // ancestor, then append the missing suffix so failures share target identity.
  let ancestor = resolved;
  const suffix: string[] = [];
  while (true) {
    try { return path.join(fs.realpathSync.native(ancestor), ...suffix).toLowerCase(); }
    catch (error) {
      if (!["ENOENT", "ENOTDIR"].includes((error as NodeJS.ErrnoException).code ?? "")) throw error;
      const parent = path.dirname(ancestor);
      if (parent === ancestor) return resolved.toLowerCase();
...
export async function runExtractor(
  ...
  try {
    ...
    for (const file of targetFiles) {
      const key = canonicalPath(file), original = path.resolve(file);
      const prior = targets.get(key);
      if (prior && prior !== original) throw new Error(`Extractor 대상 경로 별칭 충돌: ${prior} / ${original}`);
      targets.set(key, original);
    }
    ...
    for (const failure of failures) {
      ...
      if (!targets.has(key)) throw new Error(`Extractor failed-out 계약 위반: 대상 밖 실패 경로 ${failure.abs}`);
      if (seen.has(key)) throw new Error(`Extractor failed-out 계약 위반: 중복 실패 경로 ${failure.abs}`);
      seen.add(key);
      // Downstream manifest diff uses the caller's spelling, not the 8.3-expanded alias.
      failure.abs = targets.get(key)!;
    }
    ...
    for (const chunk of successfulChunks) {
      const original = targets.get(canonicalPath(chunk.abs));
      if (!original) throw new Error(`Extractor 청크 계약 위반: 대상 밖 경로 ${chunk.abs}`);
      chunk.abs = original;
    }

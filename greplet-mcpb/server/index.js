...
function backendDownMessage(e) {
  return `인덱서 요청 실패(${BASE_URL}) — 이 오류만으로 서버 미가동을 단정할 수 없습니다. 상세 원인을 확인하고, 미가동이 확인된 경우에도 기존 운영 지침·허용 범위 안에서만 기동하세요. 조회 실패는 설정 변경·재인덱싱 허가가 아닙니다. 상세: ${e instanceof Error ? e.message : String(e)}`;
}
...
server.registerTool(
  "greplet",
  {
    ...
    inputSchema: {
      ...
      all: z.boolean().default(false).describe("true면 workspace보다 우선하여 모든 워크스페이스 통합 검색, 점수순 병합. 대상이 분명하면 all=false와 workspace를 지정할 것"),

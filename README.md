...
```
[Claude Code skill / MCP / greplet.ps1 / post-commit]
                 │  HTTP (127.0.0.1:7802)
                 ▼
...
| `greplet.ps1` | PowerShell CLI 클라이언트 |
...
| OS | Windows 에서 개발·검증 (LanceDB win32-x64). 스크립트는 pwsh 기준 |
...
데이터(LanceDB·매니페스트·업로드·로그)는 `%LOCALAPPDATA%\greplet` 에 저장된다. 로그온 시 자동 기동은 Windows 작업 스케줄러에 `pwsh -File <경로>\indexer\start-indexer.ps1` 를 등록한다.
...
| `GREPLET_DATA_DIR` | `%LOCALAPPDATA%\greplet` | DB·매니페스트·업로드·로그 저장 위치 |
| `GREPLET_WORKSPACES` | `indexer/workspaces.json` | 워크스페이스 정의 파일 |
| `GREPLET_EXTRACTOR` | `Extractor/bin/Release/net8.0/Extractor.exe` | Extractor 실행 파일 |
...
| 클라이언트 | 위치 | 비고 |
|---|---|---|
| PowerShell CLI | `greplet.ps1` | `-Query -Workspace -All -TopN -Full -Mode -BaseUrl` |
